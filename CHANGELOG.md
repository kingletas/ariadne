...
- **The line down from the bookmark is drawn while you move it, and goes when you stop.** It is what shows that every strip below the axis is on the same scale — but it crosses every card on the way down, which is a permanent cost for an occasional benefit. It appears when the alignment is the thing being looked at. *Always show the bookmark line* in the menu keeps it there for anybody who wants it.
...
- **The window had no icon in the dock or the app grid.** Wayland takes a window's application id from the program name, and the launcher runs `python3 -c …` — so every window announced itself as `python3`, GNOME looked for `python3.desktop`, found nothing, and drew a generic icon on an application that was otherwise working. The program name is the application id now, and the desktop entry states its `StartupWMClass` for X11, where the match is on `WM_CLASS` instead.
- **`make uninstall` left a stale icon cache behind**, which is what actually hid the icon. It removed the icon it had installed and refreshed the desktop database but not the icon cache — and a user icon directory outranks `/usr/share`, so a cache there naming an icon that is gone hides the packaged one entirely. It refreshes both now, the way the installer writes both.
- **The icon was still in the retired palette.** The sweep that took the old colours out of the code looked only at `app/`, and the icon is the one drawing that lives outside it.
- **A folder of Markdown could not be opened at all.** Keying a book on its contents opened the book to hash it, and a folder is not a file — so choosing one on the welcome screen raised `IsADirectoryError` on a manuscript. A folder is keyed on its chapter files and what is in them, by the same rule the ingest uses, so a note left beside a manuscript is not part of it.
- **"Since chapter 1…" never went away.** It was given no timeout, so it sat over every view until it was dismissed by hand. It carries an action, so it lasts longer than an ordinary notice, and then it goes.
